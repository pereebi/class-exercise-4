...
const userOptions = async (
  ...
  const milkChoice: number = parseInt(milkPicked as string) as number;
  const espPicked = await espressoQ();
  const espChoice: number = parseInt(espPicked as string) as number;
  // If peppermint mocha
...
const main = () => {
  ...
  const buildMocha = async () => {
    do {
      ...
      menuChoice = parseInt(optionPicked as string) as number;
      switch (menuChoice) {
        case 0: {
          break;
        }
